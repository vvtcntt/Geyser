...
namespace Geyser.Controllers.Display
{
    public class OrderController : Controller
    {
        ...
        public ActionResult Command(FormCollection collection, string tag)
        {
            if (collection["btnorder"] != null)
            {
                try
                {
                    ...
                    var MaxOrd = db.TblOrder.OrderByDescending(p => p.Id).Take(1).ToList();
                    int idOrder = 1;
                    ...
                    for (int i = 0; i < Sopping.CartItem.Count; i++)
                    {
                        ...
                        db.TblOrderDetail.Add(orderdetail);
                        db.SaveChanges();
                    }
                    TblConfig config = db.TblConfig.First();
                    var fromAddress = config.UserEmail;
                    var toAddress = config.Email;
                    var orders = db.TblOrder.OrderByDescending(p => p.Id).Take(1).ToList();
                    ...
                    if (rdtt == "2")
                    {
                        chuoihtml += "<p style=\"font-size:12px; margin:5px 10px\">Hình thức thanh toán :<span style=\"font-weight:bold\">Nhận hàng và thanh toán tại Loiloc</span></p>";
                    ...
                    chuoihtml += "</div>";
                    chuoihtml += "</div>";
                    var listo = db.TblOrder.OrderByDescending(p => p.Id).Take(1).ToList();
                    ...
                    chuoihtml += "<span style=\"font-size:12px; text-align:center; color:#FFF; line-height:40px; display:block\">Copyright (c) 2002 – 2015 Loiloc VIET NAM. All Rights Reserved</span>";
                    chuoihtml += " </div>";
                    chuoihtml += "</div>";
                    chuoihtml += "</body>";
                    chuoihtml += "</html>";
        ...
        public ActionResult OrderIndex()
        {
            ...
            //Load info công ty
            var tblconfig = db.TblConfig.First();
            string chuoicty = "";
        ...
        public ActionResult DeleteOrder(int id)
        {
            ...
            return Json(new { thongtin = "", sl = sl, tt = string.Format("{0:#,#}", s.CartTotal), count = s.CartItem.Count });
        }
