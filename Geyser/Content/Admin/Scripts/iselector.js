	...
	var ISSelectReplacement = {
		...
		replace_select: function(element)
		{
			...
			if(element.ondblclick)
			{
				...
				{
					$j(this.select).trigger('dblclick');
				}
			}
		...
		on_hover: function(element, id, action)
		{
			...
			if(action == 'out') {
				$j(element).removeClass('ISSelectOptionHover');
				$j(option).trigger('mouseout');
			}
			else {
				if(!$j(element).hasClass('SelectedRow')) {
					$j(element).addClass('ISSelectOptionHover');
				}
				$j(option).trigger('mouseover');
			}
		},
		...
		on_click: function(element, id)
		{
			...
			$j(element).parents('div')[0].selectedIndex = replacement.selectedIndex;
			
			$j(option).triggerHandler('click');
			
			if (checkbox.checked) {
				$j(element).addClass('SelectedRow');
			}
			else {
				$j(element).removeClass('SelectedRow');
			}
		}
	};
